...
RectView::RectView(MODEL model,
        QWidget *parent) : QWidget(parent),
        color_(Qt::red) {
    /// 确定画图样式
    switch (model) {
    case MODEL::Arrows:
        drawFunc_ = &RectView::drawArrows;
        break;
    case MODEL::Rectangle:
        drawFunc_ = &RectView::drawRect;
        break;
    case MODEL::Roundness:
        drawFunc_ = &RectView::drawEllipse;
        break;
    default:
    {
        drawFunc_ = &RectView::drawArrows;
...
void RectView::paintEvent(QPaintEvent *) {
...
void RectView::drawArrows(const QPoint& startPoint,
...
void RectView::drawRect(const QPoint &startPoint, const QPoint &endPoint,
                           QPainter &paiter) {
    paiter.setPen(color_);
    paiter.drawRect(QRect(startPoint, endPoint));
}

void RectView::drawEllipse(const QPoint &startPoint, const QPoint &endPoint,
                              QPainter &paiter) {
    paiter.setPen(color_);
    paiter.drawEllipse(QRect(startPoint, endPoint));
}
void RectView::onColor(const QColor &color) {
    color_ = color;
}
